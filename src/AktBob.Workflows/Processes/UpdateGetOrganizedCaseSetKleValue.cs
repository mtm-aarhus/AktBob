...
internal class UpdateGetOrganizedCaseSetKleValue(IServiceScopeFactory scopeFactory, IConfiguration configuration) : IJobHandler<UpdateGetOrganizedCaseSetKleValueJob>
{
    public async Task Handle(UpdateGetOrganizedCaseSetKleValueJob job, CancellationToken cancellationToken = default)
    {
        ...
        {
            var kleValue = Guard.Against.NullOrEmpty(configuration.GetValue<Guid>("UpdateGetOrganizedCaseSetKleValue:NovaCaseKleMapping"));
    ...
    private static async Task UpdateTargetCaseKle(IGetOrganizedModule go, string caseId, Guid kleId, CancellationToken cancellationToken)
    {
