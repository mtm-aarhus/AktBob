...
internal class GetCaseMetadataHandler(IGetOrganizedClient getOrganized) : IGetCaseMetadataHandler
{
    public async Task<ErrorOr<CaseMetadataDto>> Handle(string caseId, CancellationToken cancellation = default)
    {
        var result = await getOrganized.GetCaseMetadata(caseId, cancellation);
        return result == null
